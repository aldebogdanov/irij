...
- [ ] LSP server
- [x] Dynamic/variable map keys in map literals `{(expr)= val}` — Done (PR4, 2026-07); key must evaluate to Str; also works in `{...base (k)= v}` record updates
...
- [x] **`\$` didn't suppress interpolation, and `"\\n"` mis-unescaped** — Fixed (2026-07, with the `\e` addition). Two bugs in the same code: (1) `visitString` scanned for a bare `${`, so `"\${x}"` interpolated anyway *and* left the backslash in the output, despite the lexer accepting `\$` as an escape — now it skips `${` preceded by an odd number of backslashes. (2) `unescapeString` was a chain of `String.replace` calls, so a literal backslash followed by an escape letter got mangled: `"\\n"` produced backslash + newline because the `\n` rule consumed the second backslash before the `\\` rule ran — now a single left-to-right scan, which can't re-enter. 6 tests in `StringEscapeTest`.
- [x] **`use <seed>.<mod>` could resolve to the wrong seed** — Fixed (2026-07). A seed's modules live flat in its root (`uzor.core` is `<uzorRoot>/core.irj`, no per-seed dir), so `ModuleInliner` strips the seed prefix and looks for `core.irj`. That strip was tried against *every* seed root in turn, so with two seeds that both ship a `core.irj` — uzor and butterfly, say — `use uzor.core` resolved to whichever root came first. Silent: the module loaded, and the only symptom was every name the caller wanted being undefined. The strip is now attempted only against the root that actually provides that seed, identified by the root's own `irij.toml` (falling back to its directory name, or its parent's for the installed `<name>/<version>` layout). Found trying to add butterfly to uzor for U5. 4 tests in `SeedModuleResolutionTest`, all of which fail without the fix.
- [x] **`&&` and `||` evaluated both operands** — Fixed (2026-07). They were emitted like any other binary op (evaluate left, evaluate right, call `RtOps.and`), so the universal guard idiom silently didn't guard: `(contains? m "k") && (use (get "k" m))` ran `use` regardless. Found from uzor, where `(key-event? ev) && (empty? (get "mods" ev))` crashed the app on every event without a `mods` field — which is every idle timeout tick. Now lowered to a branch like `if`, so the right operand runs only when it decides the answer; a skipped operand performs no effects either. The result stays a `Bool` rather than becoming the operand, matching the old runtime exactly. 6 tests in `PrimitivesTest`; spec §1.2 and `bytecode.md` updated.
- [x] **`role` reserved, and the `mod` builtin unreachable** — Fixed (2026-07). Two collisions between keywords and names developers actually want. (1) `role` was the choreography keyword, so `user.role`, `{role= …}` and `fn role-of` were all parse errors — it even broke the language spec's own §2.2 example, which declared `role :: Role`. The keyword is now `party` (`party $BUYER`), matching the choreography prose; `ROLE_NAME`/`RoleDecl`/`RoleRef` renamed through to `PARTY_*` so nothing is half-named. (2) `mod` is the module-decl keyword, so the `mod` builtin registered in `Builtins` could never be called — `mod 7 3` didn't parse. Integer division and remainder are now `quo` and `rem`; `std.math` re-exports both and loses the comment explaining how to reach modulo without them. The Java statics keep the names `RtOps.div`/`RtOps.mod`, since those also back `/` and `%`. 4 tests in `PrimitivesTest`, 2 in `QualifiedUseTest`.
  - Not done: soft keywords. Every remaining reserved word still breaks in every identifier position (`post`, `in`, `out`, `cap`, `select`, `for`, …) even where the grammar is unambiguous — `nameListItem` already permits all 28 inside `use` lists, so the machinery for it exists.
- [x] **Product specs checked field presence only** — Fixed (2026-07). `spec Point / x :: Int` parsed the field spec into `Decl.SpecField` and then dropped it at registration (`registerProduct` took names only), so `{x= "str" y= 2}` satisfied `Point`, and any value carrying a *superset* of the field names passed too. Field specs are now `encode`d alongside the names — so a field may be anything a fn annotation may be (`#[Int]`, a nested product, an arrow) — and products are **closed**: an undeclared field is a mismatch, not a superset that fits. The constructor got the same treatment via `SpecValidator.certifyProduct`; without it the `specName` fast-path let `Point "str" 2` through every later check. Sum specs were already discriminated (tag + arity) and are unchanged. 8 tests in `UserSpecTest`; no fallout in irij, uzor, butterfly, chereda or the examples.
  - Not done: an opt-in *open* product (Malli's `{:closed false}`), for a shape that deliberately carries extra keys. Nothing needed it yet.
- [x] **A lambda's captured parameter lost to a same-named top-level binding** — Fixed (2026-07). `LambdaEmitter.collectFreeVars` skipped any name present in `topLevelFields`, so a lambda inside a fn didn't capture the fn's own parameter when the *calling program* happened to have a global of that name — it read the global instead. No error, just a plausible wrong value. Modules inline into one namespace, so any library was breakable by an unrelated global in user code (found via uzor: a test file's `st := plain` made `uzor.style` render every span with the wrong style). A top-level binding also has a dual local slot in `main`, so name alone can't discriminate; the check is now slot identity against `ClassEmitter.topLevelLocals`. Both rules pinned in `LambdaCaptureTest`: parameters shadow globals, and a top-level lambda still sees later writes to one.
- [ ] **A top-level read can miss a static write made from a lambda** — the dual-slot hazard's other half. `counter :! 0` then `f (-> counter <- counter + 1)` updates the static field, but a subsequent top-level `counter` read uses the stale local slot and sees 0. Pre-existing, distinct from the capture bug above; probably wants the dual slot dropped entirely once nothing depends on it.
...
- [x] **Multi-arg chained lambdas** — Fixed (PR4, 2026-07): grammar `lambdaChain` rule; `(a -> b -> body)` nests right. Previously: — `(acc -> pair -> body)` fails to parse. The grammar's `lambdaExpr` rule is `LPAREN lambdaParams ARROW exprSeq RPAREN` where `lambdaParams` is `pattern*`, so `(a b -> body)` works (multi-param), but curried `(a -> b -> body)` requires explicit nesting: `(a -> (b -> body))`. Consider whether the grammar should support `->` chaining or if explicit nesting is the intended design.
- [x] **`with handler` block followed by trailing `if/else` body drops the return value.** Fixed in v0.8.4. EffIR lowerer now enters its branching path when the body's last stmt is an IfStmt preceded by an op-bearing segment; branches lower with `null` exitJump so each tail ExprStmt becomes a Return instead of jumping to an unfinalized merge block.
- [x] **Snake_case field dot-access blows up bytecode emit** with "MVP: unsupported expression: Wildcard". Fixed in v0.8.4. IDENT rule now accepts `_` as a separator alongside `-` (`[a-z] [a-z0-9]* (('-' | '_') [a-z0-9]+)* [?!]?`), so SQL/JSON field names like `pw_hash`, `user_id`, `session_token_hash` lex as a single IDENT. Bare `_` and leading-`_` still tokenize as Wildcard so pattern matching is unchanged.
- [x] **`with` body ending in a block-form `if`/`else` still dropped the value when a branch had bindings or performed an op** — the v0.8.4 fix only covered *pure single-expression* branches. Two remaining gaps, both fixed in **v0.8.7**: (1) EffIR — a tail `IfStmt` whose branches perform ops fell through to the op-bearing *merge-block* case, whose merge returned `null`; the tail-position check now runs **before** the merge case so each branch's tail expr becomes a `Return`. (2) Pure/SingleOp/Sequence — `emitBlockStmtsReturning` (and the handler-join / `on-failure` emitters) only treated a tail `ExprStmt` as a value, so a block-form tail `if`/`match` returned Unit; all now route through the shared `emitTailStmtValue` helper (mirrors `emitBlock`). Inline `if c a else b` (an `IfExpr`) always worked — only the multi-line block form was broken. This is what white-screened the irij.online seed-detail page (`with default-db { row := db-query …; if (empty? row) … else { rows := db-query …; render … } }` returned an empty body). 3 regression tests in `StateMachineWithTest`.
