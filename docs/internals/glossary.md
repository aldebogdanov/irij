...
| **BuiltinFn** | Interpreter-side Java-implemented function value. Effect-transparent — callback runs in caller's row. |
...
| **EffectSystem.STACK** | Thread-local stack of `HandlerContext` records. Used by the threaded protocol (`fireOp`). |
...
| **fireOp** | `EffectSystem.fireOp(eff, op, args)` — threaded protocol's effect-op invocation. Sends a message on the op channel, blocks for resume. Falls through to `fireOpToSM` for SM handlers. |
| **fireOpToSM** | Synchronous SM-dispatch from `fireOp` — used when a fiber (running outside any SM dispatch loop) needs to reach an inherited SM handler. |
...
| **nREPL** | Network REPL — Clojure-flavoured protocol. Irij hosts an nREPL server with interpreter-backed sessions. |
| **OpSection** | `(+)` etc. as a first-class fn value. Lowered to `GETSTATIC RuntimeSupport.OP_ADD` etc. |
| **Perform** | An effect-op invocation. In threaded mode: `fireOp` → channel; in SM mode: throw `PerformSignal`. |
...
| **ResumeChannel** | Per-perform `SynchronousQueue<Object>` used by the threaded protocol to send the resume value from clause back to body. |
| **runHandlerLoop** | Calling-thread loop in threaded mode. Reads from op channel, dispatches clauses, returns body result. |
