...
0.6.7 &mdash; R5a continuing groundwork: runtime builtin registry. `RuntimeSupport.builtinFn(name)` lazily wraps every `BuiltinFn` registered by `Builtins.install` into an `IrijFn`. `emitVarLoad`'s "Unbound variable" fallback emits `INVOKESTATIC RT.builtinFn("name")` so any builtin reaches first-class value position without per-name enumeration. Plus bug fixes uncovered while attempting the CLI flip: `nth idx coll` arg order corrected (was `coll idx`); `RT.cmp` delegates to `Builtins.compare` so sort handles tuples / vectors / keywords; `nth` accepts `IrijTuple`. CLI flip itself remains held — `irij run`/`test` still on interp pending bytecode emit for top-level `MutBind`, broader `IrijRange` support across HOFs, and a few else-if value-propagation edge cases. Plus 0.6.6.
